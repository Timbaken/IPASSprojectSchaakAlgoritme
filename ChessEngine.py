...
class GameState:
    def __init__(self, white_to_move = True, customCastling = [True, True, True, True],   board = [
        ...
        self.current_castling_rights = CastleRights(customCastling[0], customCastling[1], customCastling[2], customCastling[3])
    ...
    def getKingsideCastleMoves(self, row, col, moves):
        # if self.castle_rights_log[-1].bks or self.castle_rights_log[-1].wks:
        if self.board[row][col + 1] == '--' and self.board[row][col + 2] == '--':
            if not self.squareUnderAttack(row, col + 1) and not self.squareUnderAttack(row, col + 2):
                moves.append(Move((row, col), (row, col + 2), self.board, is_castle_move=True))

    def getQueensideCastleMoves(self, row, col, moves):
        # if self.castle_rights_log[-1].bqs or self.castle_rights_log[-1].wqs:
